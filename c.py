...
def shunt(xs):
    ...
    for i in range(1, len(xs) - 1, 2):
        ...
        assoc, right = 0, 0
        if isinstance(H, Token) and H.v in ASSOC:
            assoc, right = ASSOC[H.v]
...
def _finalize(buf, leading_punct, start_paren, end_paren, quote):
    ...
    if len(buf) == 0:
        ...
        buf = [Var(x.v) if isinstance(x, Token) else x for x in buf]
        buf = shunt(buf)
    elif start_paren == "{":
        buf = pad_buf(leading_punct, buf)
        buf[0] = Var(buf[0].v) if isinstance(buf[0], Token) else buf[0]
        buf = shunt(buf)
    elif start_paren == "[":
        if quote:
            buf = pad_buf(leading_punct, buf)
...
def parse(i, start_paren, quote, cs, xs):
    ...
    while i[0] + 1 < len(xs):
        ...
        if new_token != token and token is not None:
            ...
            cs_ = "".join(cs)
            if token == "symbol" and all(x.isnumeric() for x in cs_):
                cs_ = Number(cs_)
            else:
                cs_ = Token(cs_)
